        ...
        @foreach ($msmeType::cases() as $type)
            @php
                $icon = match ($type) {
                    ...
                    $msmeType::BUMDES_PARTNERSHIP => 'ri-leaf-line',
                };
            @endphp

            <a href="{{ route('statistik.msme', ['type' => $type->value, 'rw' => request('rw'), 'rt' => request('rt')]) }}"
                class="max-md:w-full">
                <x-buttons.filter-button :icon="$icon" :active="request('type') === $type->value ||
                    (!request('type') && $type === $msmeType::BUSINESS_SECTOR)"
