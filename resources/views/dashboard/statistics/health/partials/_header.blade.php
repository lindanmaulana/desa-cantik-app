<div class="flex items-center gap-3 pb-4 mb-6 border-b sm:gap-4 sm:pb-6 sm:mb-10 border-textTertiary/30">
    ...
    <div>
        <h3 class="text-lg font-bold sm:text-xl text-textPrimary">Kesehatan</h3>
        <p class="text-sm sm:text-lg text-textSecondary">Analisis agregat kesehatan {{ $villageSettings->village_name ?? '-' }}</p>
    </div>
</div>
...
<div class="space-y-4" x-show="!isGenerated">
    <div
        ...
        <form action="{{ route('dashboard.statistics.health') }}" method="GET"
            ...
            {{-- Mengasumsikan STUNTING_STATUS atau jenis enum kesehatan pertama Anda sebagai default value --}}
            <input type="hidden" name="type" value="{{ $healthType::STUNTING_STATUS->value }}">

            <x-ri-heart-pulse-line class="p-3 mb-2 rounded-md sm:mb-4 size-12 sm:size-16 bg-primary/10 text-primary" />

            <h4 class="text-lg font-bold sm:text-xl text-textPrimary">Hitung Agregat Kesehatan</h4>

            <p class="px-2 mb-4 text-xs sm:text-base text-textSecondary">
                Sistem akan memproses seluruh data untuk menghasilkan statistik Kesehatan. Proses ini hanya dilakukan
                sekali per sesi.
            </p>
        ...
        <div x-show="isGenerating" x-cloak x-transition:enter="transition ease-out duration-300"
            ...
            <div class="w-full max-w-xs space-y-2 text-center">
                ...
                <p class="text-xs font-medium text-textSecondary animate-pulse">Mengalkulasi profil kesehatan, KB, & gizi balita warga...</p>
            </div>
        </div>
