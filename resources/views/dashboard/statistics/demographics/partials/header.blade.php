    ...
    <div
        ...
        <div class="flex flex-col items-center justify-center max-w-md gap-2 text-center transition-all duration-300"
            ...
            <p class="px-2 mb-4 text-xs sm:text-base text-textSecondary">
                Sistem akan memproses seluruh data untuk menghasilkan statistik Demografi. Proses ini hanya dilakukan
                sekali per sesi.
            </p>

            <button type="button"
                @click="isGenerating = true; setTimeout(() => { window.location.href = '{{ route('dashboard.statistics.demograph', ['type' => 'ageGroup']) }}' }, 2500)"
        ...
        <div x-show="isGenerating" x-cloak x-transition:enter="transition ease-out duration-300"
            ...
            <div class="w-full max-w-xs space-y-2 text-center">
                ...
                <p class="text-xs font-medium text-textSecondary animate-pulse">Mengalkulasi data demografi warga...</p>
            </div>
        </div>
