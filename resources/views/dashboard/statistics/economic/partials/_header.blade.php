<div class="flex items-center gap-3 pb-4 mb-6 border-b sm:gap-4 sm:pb-6 sm:mb-10 border-textTertiary/30">
    <x-phosphor-money
        class="p-2 rounded-md sm:p-3 size-10 sm:size-12 bg-primary/10 text-primary shrink-0" />
    <div>
        <h3 class="text-lg font-bold sm:text-xl text-textPrimary">Ekonomi</h3>
        <p class="text-sm sm:text-lg text-textSecondary">Analisis ekonomi & aset {{ $villageSettings->village_name ?? '-' }}</p>
    </div>
</div>
...
<div class="space-y-4" x-show="!isGenerated"
    ...
    <div class="flex items-center justify-center border-2 border-dashed rounded-2xl border-textTertiary/60 bg-secondary min-h-[300px] sm:min-h-96 p-4 shadow-sm relative overflow-hidden">

        {{-- Mengubah container menjadi FORM untuk trigger loading dinamis --}}
        <form action="{{ route('dashboard.statistics.economic') }}" method="GET"
            ...
            {{-- Input hidden untuk mengirim parameter type enum ekonomi --}}
            <input type="hidden" name="type" value="{{ $economicType::OCCUPATION->value }}">

            <x-phosphor-money
                class="p-3 mb-2 rounded-md sm:mb-4 size-12 sm:size-16 bg-primary/10 text-primary" />

            <h4 class="text-lg font-bold sm:text-xl text-textPrimary">Hitung Agregat Ekonomi</h4>
            ...
            {{-- Tombol submit dengan pemblokir double-click --}}
            <button type="submit"
                ...
                {{-- Spinner SVG Real-time --}}
                <svg x-show="isGenerating" x-cloak class="animate-spin size-5 text-secondary" viewBox="0 0 24 24" fill="none">
        ...
        <div x-show="isGenerating" x-cloak x-transition:enter="transition ease-out duration-300"
            ...
            <div class="w-full max-w-xs space-y-2 text-center">
                <div class="h-1.5 w-full bg-tertiary rounded-full overflow-hidden relative">
                    <div class="h-full bg-primary rounded-full w-full origin-left animate-[loading_1.5s_infinite_ease-in-out]">
                    </div>
                </div>
                <p class="text-xs font-medium text-textSecondary animate-pulse">Mengalkulasi aset & data ekonomi desa...</p>
            </div>
        </div>
