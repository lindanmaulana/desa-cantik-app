<div class="flex items-center gap-3 pb-4 mb-6 border-b sm:gap-4 sm:pb-6 sm:mb-10 border-textTertiary/30">
    <x-ri-map-pin-line class="p-2 rounded-md sm:p-3 size-10 sm:size-12 bg-primary/10 text-primary shrink-0" />
    <div>
        <h3 class="text-lg font-bold sm:text-xl text-textPrimary">Data Spasial</h3>
        <p class="text-sm sm:text-lg text-textSecondary">Analisis agregat pemetaan geografis dan tata ruang
...
<div class="space-y-4" x-show="!isGenerated">
    <div
        ...
        <form action="{{ route('dashboard.statistics.spatial-data') }}" method="GET" @submit="isGenerating = true"
            ...
            <x-ri-map-2-line class="p-3 mb-2 rounded-md sm:mb-4 size-12 sm:size-16 bg-primary/10 text-primary" />

            <h4 class="text-lg font-bold sm:text-xl text-textPrimary">Hitung Agregat Spasial</h4>

            <p class="px-2 mb-4 text-xs sm:text-base text-textSecondary">
                Sistem akan memproses dan mengelompokkan seluruh data koordinat untuk menghasilkan statistik spasial wilayah. Proses ini hanya dilakukan sekali per sesi.
            </p>
        ...
        <div x-show="isGenerating" x-cloak x-transition:enter="transition ease-out duration-300"
            ...
            <div class="w-full max-w-xs space-y-2 text-center">
                ...
                <p class="text-xs font-medium text-textSecondary animate-pulse">Mengalkulasi persebaran infrastruktur, zona UMKM, & batas pemukiman warga...</p>
            </div>
        </div>
