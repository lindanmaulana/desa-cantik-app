<div class="overflow-hidden border shadow-sm bg-secondary border-textTertiary/30 rounded-xl">
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            <thead>
                <tr
                    class="text-xs font-semibold tracking-wider uppercase border-b text-textSecondary border-textTertiary/20 bg-tertiary/40">
                    ...
                    @foreach ($users as $item)
                        <tr class="transition-colors hover:bg-tertiary/30">
                            ...
                            <td class="px-6 py-4 max-md:px-3 max-md:py-2">
                                <span
                                    class="inline-flex items-center px-2 py-1 text-xs font-semibold tracking-wider uppercase rounded-full bg-primary/10 text-primary">
                                    {{ $item->role }}
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-4 text-center max-md:px-3 max-md:py-2">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <button type="button" @click.stop.prevent="openDeleteModal($event)"
                                        data-url="{{ route('admin.manage-operator.destroy', $item->id) }}"
                                    ...
                                    {{-- <form action="{{ route('admin.manage-operator.destroy', $item->id) }}"
    ...
    <div
        ...
        <p>Menampilkan {{ $users->firstItem() ?? 0 }} sampai {{ $users->lastItem() ?? 0 }} dari {{ $users->total() }}
            admin</p>
