<div class="grid grid-cols-1 gap-x-8 gap-y-6 md:grid-cols-3">
    <div>
        ...
        <p class="mt-1 text-sm leading-6 text-gray-500">Aset gambar visual transparan lambang desa. Disarankan berformat
            PNG atau SVG.</p>
    </div>
    <div class="md:col-span-2">
        <form action="{{ route('settings.update-logo') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="p-6 sm:flex sm:items-center sm:gap-x-6">
                <div
                    class="flex items-center justify-center w-24 h-24 p-2 border border-gray-200 shadow-inner shrink-0 rounded-xl bg-gray-50">
                    @if (!empty($settings->village_logo))
                        <img src="{{ asset('storage/' . $settings->village_logo) }}"
                            class="object-contain max-w-full max-h-full" alt="Logo">
                    @else
            ...
            <div class="flex items-center justify-end px-6 py-3 border-t border-gray-100 gap-x-6 bg-gray-50">
                <button type="submit" x-bind:disabled="submitting"
                    ...
                    <span x-text="submitting ? 'Memperbaharui...' : 'Perbarui Logo Resmi'"></span>
                </button>
            </div>
        </form>
    </div>
