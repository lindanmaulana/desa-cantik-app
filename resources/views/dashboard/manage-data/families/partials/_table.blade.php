<div class="overflow-hidden border shadow-sm bg-secondary border-textTertiary/30 rounded-xl">
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="text-sm divide-y text-textPrimary divide-textTertiary/20 max-md:text-xs">
                ...
                <tr class="transition-colors hover:bg-tertiary/30">
                    ...
                    <td class="max-w-xs px-6 py-4 truncate text-textSecondary max-md:px-3 max-md:py-2">
                        {{ $item->address_detail }}
                    </td>
                    <td class="px-6 py-4 font-medium text-center max-md:px-3 max-md:py-2">
                        {{ $item->citizens->count() }} orang
                    </td>
                    <td class="px-6 py-4 text-center max-md:px-3 max-md:py-2">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            <a href="{{ route('dashboard.manage-data.families.detail', $item) }}"
                                class="p-1.5 text-primary hover:bg-primary/10 rounded-md transition-colors"
                                title="Lihat Detail">
                            ...
                            <button type="button" @click.stop.prevent="openDeleteModal($event)"
                                data-url="{{ route('families.destroy', $item->id) }}"
