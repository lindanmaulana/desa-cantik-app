<div class="overflow-hidden bg-white border border-gray-100 shadow-sm rounded-xl">
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="text-sm text-gray-700 divide-y divide-gray-100 max-md:text-xs">
                @if($families->isNotEmpty())
                @php $no = $families->firstItem(); @endphp
                @foreach($families as $item)
                <tr class="transition-colors hover:bg-gray-50/70">
                    <td class="px-6 py-4 font-medium text-center text-gray-400 max-md:px-3 max-md:py-2">{{ $no++ }}</td>
                    <td x-show="openData" class="px-6 py-4 font-semibold text-gray-900 max-md:px-3 max-md:py-2">{{ $item->family_card_number }}</td>
                    <td x-show="!openData" class="px-6 py-4 font-semibold text-gray-900 max-md:px-3 max-md:py-2"><x-vaadin-ellipsis-h class="size-5 text-slate-400" /></td>
                    <td class="px-6 py-4 text-gray-600 max-md:px-3 max-md:py-2">
                        @if($item->territory)
                    ...
                    <td class="max-w-xs px-6 py-4 text-gray-500 truncate max-md:px-3 max-md:py-2">{{ $item->address_detail }}</td>
                    <td class="px-6 py-4 font-medium text-center max-md:px-3 max-md:py-2">{{ $item->citizens->count() }} orang</td>
