{{-- Modal Konfirmasi Hapus Data UMKM --}}
<div x-show="openDelete"
    ...
    <div @click.away="!submitting && (openDelete = false)"
        x-show="openDelete"
        x-transition:enter="transition ease-out duration-300"
            ...
            <div class="flex items-start gap-4">
                ...
                <div>
                    <h3 class="mb-1 text-lg font-bold text-textPrimary">
                        Hapus Profil Usaha UMKM
                    </h3>
                    <p class="text-sm leading-relaxed text-textSecondary">
                        Apakah Anda yakin ingin menghapus profil usaha <span class="font-semibold text-textPrimary" x-text="msme.business_name"></span> secara permanen? Tindakan ini akan menghapus riwayat omset, data kemitraan, serta status perizinan yang tercatat. <span class="font-semibold text-red-500">Aksi ini tidak dapat dibatalkan</span>.
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end gap-3 px-6 py-4 border-t bg-tertiary/40 border-textTertiary/20">
            ...
            <form :action="deleteRoute" method="POST" class="inline" @submit="submitting = true">
                ...
                <button type="submit"
                    x-bind:disabled="submitting"
                    class="inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-lg shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 disabled:opacity-60 disabled:pointer-events-none min-w-[140px] dark:focus:ring-offset-slate-900 transition-all">

                    <svg x-show="submitting" x-cloak class="w-4 h-4 text-white animate-spin" viewBox="0 0 24 24" fill="none">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
                    </svg>

                    <span x-text="submitting ? 'Memproses...' : 'Ya, Hapus UMKM'"></span>
                </button>
            </form>
        </div>
    </div>
