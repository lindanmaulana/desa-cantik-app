<form action="{{ route('dashboard.manage-data.territories') }}" method="GET"
    class="flex flex-col items-center justify-between gap-4 p-4 max-md:p-3 mb-6 bg-secondary border border-textTertiary/30 shadow-sm rounded-xl md:flex-row">
    <div class="relative w-full md:w-72">
        ...
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama dusun atau wilayah..."
            class="w-full py-2 pl-10 pr-4 text-sm max-md:text-xs bg-secondary border border-textTertiary/40 rounded-lg text-textPrimary placeholder:text-textSecondary/60 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary">
    </div>

    <div class="flex items-center justify-end w-full gap-2 md:w-auto">
        <select name="sub_village" onchange="this.form.submit()"
            class="w-full px-3 py-2 text-sm max-md:text-xs text-textPrimary bg-secondary border border-textTertiary/40 rounded-lg md:w-44 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary">
        ...
        @if (request()->anyFilled(['search', 'sub_village', 'rt', 'rw']))
            <a href="{{ route('dashboard.manage-data.territories') }}"
                class="px-4 py-2 text-sm max-md:text-xs font-medium text-white bg-red-500 border border-transparent rounded-lg hover:bg-red-400">
