  <div class="overflow-hidden bg-white border border-gray-100 shadow-sm rounded-xl">
          ...
          <table class="w-full text-left border-collapse">
              <thead>
                  <tr class="text-xs font-semibold tracking-wider text-gray-500 uppercase border-b border-gray-100 bg-gray-50">
                      <th class="w-16 px-6 py-4 max-md:px-3 max-md:py-2 truncate text-center">No</th>
                      <th class="px-6 py-4 max-md:px-3 max-md:py-2 truncate">Tipe Objek Spasial</th>
                      <th class="px-6 py-4 max-md:px-3 max-md:py-2 truncate">Nama Objek Terkait</th>
                      <th class="px-6 py-4 max-md:px-3 max-md:py-2 truncate text-center">Garis Lintang (Latitude)</th>
                      <th class="px-6 py-4 max-md:px-3 max-md:py-2 truncate text-center">Garis Bujur (Longitude)</th>
                      <th class="px-6 py-4 max-md:px-3 max-md:py-2 truncate text-center">GeoJSON Geometry</th>
              ...
              <tbody class="text-sm text-gray-700 divide-y divide-gray-100">
                  ...
                  <tr class="transition-colors hover:bg-gray-50/70">
                      ...
                      <td class="px-6 py-4 max-md:px-3 max-md:py-2 font-mono text-center text-gray-700">{{ number_format($item->latitude, 8) }}</td>
                      <td class="px-6 py-4 max-md:px-3 max-md:py-2 font-mono text-center text-gray-700">{{ number_format($item->longitude, 8) }}</td>
                              ...
                              {{-- <form action="{{ route('spatial-data.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus titik koordinat GIS ini?');">
                                  ...
                                  <button type="submit" class="p-1.5 text-red-600 hover:bg-red-50 rounded-md transition-colors" title="Hapus Spasial">
                                      <x-heroicon-o-trash class="w-4 h-4" />
                                        ...
                                        data-url="{{ route('spatial-data.destroy', $item->id) }}"
      ...
      <div class="flex items-center justify-between px-6 py-4 max-md:px-3 max-md:py-2 text-xs text-gray-500 border-t border-gray-100 bg-gray-50/50">
