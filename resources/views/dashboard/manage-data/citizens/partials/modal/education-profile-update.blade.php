<div x-show="educationProfile.openUpdate"
    ...
    <div
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-textTertiary/10 bg-tertiary">
            ...
            <button @click="educationProfile.openUpdate = false"
                class="p-1 text-textSecondary transition-colors rounded-lg hover:text-textPrimary hover:bg-textTertiary/20">
                <x-heroicon-o-x-mark class="w-5 h-5" />
            </button>
        </div>

        <!-- Form Update Data Pendidikan -->
        <form action="{{ route('education-profile.update', $citizen) }}" method="POST"
            ...
            <div class="space-y-4">
                <!-- Jenjang Pendidikan -->
                <div>
                    <label for="update_education_level"
                    ...
                    <select id="update_education_level" name="education_level" required
                        x-model="educationProfile.data.education_level"
                ...
                <!-- Ijazah Tertinggi -->
                <div>
                    <label for="update_highest_diploma"
                    ...
                    <select id="update_highest_diploma" name="highest_diploma" required
                        x-model="educationProfile.data.highest_diploma"
                        class="w-full px-3 py-2 text-sm transition-all border border-textTertiary/40 rounded-lg bg-tertiary text-textPrimary focus:outline-none focus:bg-secondary focus:border-primary focus:ring-1 focus:ring-primary">
                        @foreach ($educationLevel::cases() as $val)
                            <option value="{{ $val->value }}" class="bg-secondary">{{ $val->label() }}</option>
                        @endforeach
                ...
                <div>
                    <label for="update_school_participation"
                    ...
                    <select id="update_school_participation" name="school_participation" required
                        x-model="educationProfile.data.school_participation"
                        class="w-full px-3 py-2 text-sm transition-all border border-textTertiary/40 rounded-lg bg-tertiary text-textPrimary focus:outline-none focus:bg-secondary focus:border-primary focus:ring-1 focus:ring-primary">
            ...
            <div class="flex items-center justify-end gap-2 pt-4 border-t border-textTertiary/10">
                <button type="button" @click="educationProfile.openUpdate = false"
